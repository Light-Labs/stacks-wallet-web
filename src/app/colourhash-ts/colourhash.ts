...
export function colourhash(hash: Uint8Array): ColourHash {
  if (hash.byteLength !== 32) throw new Error('colourhash: expects exactly 32 bytes');
...
export function colourhash_to_svg(hash: Uint8Array, options: ColourHashSvgOptions): string {
  if (hash.byteLength !== 32) throw new Error('colourhash_to_svg: expects exactly 32 bytes');
  ...
  if (colours.version !== requested_version)
    throw new Error(
      `colourhash_to_svg: unexpected version ${requested_version}, result version: ${colours.version}`
    );
  let svg = `<svg viewbox="0 0 ${elements_per_row * size - spacing} ${
    rows * size - spacing
  }" xmlns="http://www.w3.org/2000/svg"><!-- Ryder ColourHash version ${colours.version} -->`;
  let index = 0;
  for (let y = 0; y < rows; ++y)
    for (let x = 0; x < elements_per_row && index < colours.data.length; ++index, ++x)
      svg += `<path d="M ${x * size} ${y * size} ${
        shapes[colours.data[index].symbol % shapes.length]
      }" fill="${palette[colours.data[index].colour % palette.length]}"/>`;
  svg += `</svg>`;
  return svg;
}
